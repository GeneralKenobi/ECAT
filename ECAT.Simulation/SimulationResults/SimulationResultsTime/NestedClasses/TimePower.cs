		...
		private class TimePower : PowerCache<ITimeDomainSignal>, IPowerDB
		{
			...
			private bool TryConstructPower(ITwoTerminal twoTerminal, out ITimeDomainSignal power, bool voltageBA)
			{
				...
				if (_VoltageDrops.TryGet(twoTerminal, out var voltage, voltageBA))
				{
					...
					if(current != null)
					{
						var result = IoC.Resolve<ITimeDomainSignalMutable>(voltage.Samples, voltage.Step, voltage.StartSample, IoC.Resolve<ISIUnits>().PowerShort);

						// The result is a product of voltage and current waveforms, calculate power generated by each source separately
						foreach(var waveform in voltage.ComposingWaveforms)
						{
							...
							result.AddWaveform(waveform.Key, waveform.Value.MergeSelect(current.Waveform, (x, y) => x * y));
						}
