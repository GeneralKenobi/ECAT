...
namespace ECAT.Simulation
{
	...
	public class WaveformPartialState : GenericPartialStates<WaveformState, IList<double>>
	{
		...
		public IEnumerable<KeyValuePair<int, ITimeDomainSignal>> PotentialsToTimeDomainSignals(double timeStep, bool includeReferenceNode)
		{
			...
			{
				result.Add(index, IoC.Resolve<ITimeDomainSignalMutable>(pointsCount, timeStep, IoC.Resolve<ISIUnits>().VoltageShort));
			}
			...
			if(includeReferenceNode)
			{
				...
				result.Add(AdmittanceMatrixFactory.ReferenceNode, IoC.Resolve<ITimeDomainSignalMutable>(pointsCount, timeStep, IoC.Resolve<ISIUnits>().VoltageShort));
			}
		...
		public IEnumerable<KeyValuePair<int, ITimeDomainSignal>> CurrentsToTimeDomainSignals(double timeStep)
		{
			...
			{
				result.Add(index, IoC.Resolve<ITimeDomainSignalMutable>(pointsCount, timeStep, IoC.Resolve<ISIUnits>().CurrentShort));
			}
